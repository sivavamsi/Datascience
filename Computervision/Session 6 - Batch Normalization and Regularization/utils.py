...
def train(model, device, train_loader, optimizer, l1_factor):
    model.train()
...
def run(EPOCHS,reg,dropout,train_loader,test_loader):
    ...
    if reg['bn'] == 'GBN':
        ...
        bn = nn.BatchNorm2d
    model = Net(bn, dropout).to(device)
    optimizer = optim.SGD(model.parameters(), lr=0.01, momentum=0.9, weight_decay=reg["l2"])
    for epoch in range(EPOCHS):
        print("EPOCH:", epoch)
        model_,train_acc,train_losses = train(model, device, train_loader, optimizer, l1_factor=reg["l1"])
